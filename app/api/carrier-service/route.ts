...
export async function POST(req: NextRequest) {
  let tariffs: any[][];
  try {
    const { data, error } = await supabase.storage.from(process.env.SUPABASE_BUCKET!).download('tariffs.xlsx');
    if (error || !data) throw new Error('Tariffe non disponibili su Supabase!');

    const buffer = Buffer.from(await data.arrayBuffer());
    const workbook = xlsx.read(buffer, { type: 'buffer' });
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    tariffs = xlsx.utils.sheet_to_json(sheet, { header: 1 });
  } catch (e) {
    return NextResponse.json({
      error: "Tariffe non disponibili. Devi prima caricare il file Excel.",
  ...
  if (headersRowIdx === -1) {
    return NextResponse.json({
      error: "Impossibile trovare le colonne Provincia, Peso, Prezzo nel file Excel.",
      headers: tariffs[0]
    }, { status: 400 });
  }
