...
// 1) Mappa sigla → nome esteso di tutte le province italiane
...
export async function POST(req: NextRequest) {
  try {
    // Carica le tariffe da public/tariffs.json
    ...
    // Leggi e logga payload
    const payload = await req.json();
    console.log("➤ carrier-service payload:", JSON.stringify(payload));

    // Estrai provincia da destination
    const rawProv = (payload.destination?.province || "").toString().trim();
    if (!rawProv) {
      return NextResponse.json({ error: "Provincia non valida" }, { status: 400 });
    }
    // Converti sigla a nome esteso o prendi direttamente il nome esteso
    ...
    const pesoTotaleKg =
      items.reduce(
        (sum: number, i: any) => sum + (Number(i.grams) || 0) * (i.quantity || 1),
        0
      ) / 1000;
    if (pesoTotaleKg <= 0) {
    ...
    // Filtra e ordina tariffe per provincia
    const list = tariffs
    ...
    // Calcolo bancali con soglia 1000kg
