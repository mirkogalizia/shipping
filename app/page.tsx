...
export default function Dashboard() {
  ...
  // Raggruppa dati per regione
  const groupedMap: Record<Region, Tariffa[]> = {
    [Region.NORD]: [],
    [Region.CENTRO]: [],
    [Region.SUD]: [],
    [Region.ISOLE]: []
  ...
  data.forEach(t => {
    const code = t.Provincia.split(' ')[0].toUpperCase();
    const reg = regionMap[code] || Region.CENTRO;
    groupedMap[reg].push(t);
  });
  ...
  return (
    <div className="p-8 space-y-8 bg-gray-50 min-h-screen">
      <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
        ...
        <input type="file" accept=".xlsx,.xls" onChange={handleFileUpload} className="w-full border rounded p-2 mb-4" />
        <div className="flex space-x-4 mb-6">
          <input value={provincia} onChange={e => setProvincia(e.target.value)} placeholder="Provincia (MI)" className="border rounded p-2 flex-1" />
          <input value={peso} onChange={e => setPeso(e.target.value)} placeholder="Peso kg" type="number" className="border rounded p-2 w-24" />
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        {Object.entries(groupedMap).map(([reg, list]) => (
          list.length > 0 && (
